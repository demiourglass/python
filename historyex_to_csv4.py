...
component_paths = {}
historyex_types = {'1':'Create','2':'Execute','4':'Scan','6':'Quarantine','7':'Quarantine','22':'Move','40':'Open'} #event.h
...
for row in c_path_history.execute("SELECT path, hash, lastref, type FROM path_history ORDER by lastref"):
  ...
  print "{},{},{},{}".format(str_path, row[1],datetime.datetime.strptime(time.ctime(row[2]), "%a %b %d %H:%M:%S %Y"),history_ex_type)
